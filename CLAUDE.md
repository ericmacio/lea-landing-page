# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Project Overview
Ce projet a pour but de créer une landing paage présentant une offre de conseil en e-commerce et plus précisémment une offre spécialisé en CRO (Optimisation du taux de conversion d'un site).
Cette page doit présenter une Unique Value Proposition (UVP) autour de cette activité. Elle doit permettre de faire connaitre l'acitivité à tous les clients potentiels (PME, TPE, startup) en mettant l'accent sur ce que celà puet leur rapporter en terme de business (par exemple augmentation des ventes).
Toutes les documentations relatives à la création de la page sont stockés sous le répertoire "docs"

## langage et style
Cette page doit être rédigé uniquement en français. Merci d'adopter un style permettant d'attirer l'attention mais tout en restant professionnel et respectueux. La page doit être très professionelle, au look moderne et agréable permettant de mettre en évidence tous les points forts de l'offre

## Format
Cette page doit se présenter sous la forme d'une page simple au format HTML. Eviter si possible l'utilisation de JavaScript

## Codes couleur
Merci d'utiliser la palette de couleurs suivante:
 #0f0f0e;
  #7a7870
  #00d4ff
  #1a1a18
  #ffffff
  #a3a198
  #45cdff3d

La couleur de background des parties plus claires de la page doit être #eef2f6 et non pas blanches
La couleur de background des parties sombres de la page doit être #1a1a18 et non pas noires

## Fonts
Les éléments principaux (titres) utilisent la police "Instrument Serif". Les autres textes la font: "DM Sans"

## photo
La photo de la personne proposant cette offre est accessible dans le fichier lea.png

## Status
la page index.html a été créée suivant les documents stockés sous le répertoire analyse. Celle-ci est désormais entièrement opérationnelle. Elle a aussi été améliorée pour un impact visuel plus fort. Ces fichiers servent de références quand à la struture et l'état de la page actuelle. Le but maintenant est juste de procéder à quelques améliorations fonctionnelles ou visuelles, ainsi qu'à l'optimisation de son référencement par Google sur les mots clefs, CRO, optimisation site web, amélioration taux de conversion, test A/B ainsi que tout ce qui est en rapport avec l'activite